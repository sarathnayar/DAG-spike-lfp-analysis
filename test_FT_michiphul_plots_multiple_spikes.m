...
for i = 1:numel(spikeLockProbs)
    ...
    for j = 1:numel(spikeRates)
        ...
        disp(sprintf('mean rate %.2f, mean locked rate %.2f %d spikes',mean(sum(Spikes,2)/cfg.trllen),mean(sum(lockedSpikes,2)/cfg.trllen),sum(sum(Spikes))));
...
% plots
c = colormap(cool(numel(spikeLockProbs)));
for i = 1:numel(spikeLockProbs)
    % plot ppc vs. spike rate
    figure(1); hold on
    plot(spikeRates, ppc(i, :), 'o-', 'Color', c(i,:), 'MarkerFaceColor', c(i,:))
    ...
    figure(2); hold on
    plot(spikeRates, coh(i, :), 'o-', 'Color', c(i,:), 'MarkerFaceColor', c(i,:))
