...
% spikes
spikeRate = [100]; % Hz, overall spike rate
...
% find the number of locked spikes and locked spike rate
nLockedSpikes = spikeRate*spikeLockProb*cfg.trllen;
lockedSpikeRate = nLockedSpikes / cfg.trllen;
for t = 1:cfg.numtrl,
	% for each trial, find the phase (of the trough) of the corresponding freq component s
	 ...
	 % find number of troughs
     ntroughs = length(trough_idx);
     ...
     if lockedSpikeRate > lfp_freq(spikePhaseFreq)
        % allow multiple spikes to be locked to each trough with a minimal
        % interspike interval of 1 ms
        % find the number of consecúitve spikes locked to each trough
        nConsSpikes = nLockedSpikes / ntroughs;
        % now distribute the locked spikes among the troughs
        k = 1;
        multiLockedSpikes = zeros(length(taxis), 1);
        for m = 1:length(taxis)
            if m == lockedSpikes_idx(k)
                % id of consecutive spikes
                consSpikes_idx = ceil(-(floor(nConsSpikes)-1)/2:(floor(nConsSpikes)-1)/2);
                %consSpikes_idx = ig_limit_range_min_max(consSpikes_idx,0,cfg.fsample*cfg.trllen);
                multiLockedSpikes_idx = consSpikes_idx + m;
                multiLockedSpikes_idx = ig_limit_range_min_max(multiLockedSpikes_idx,1,cfg.fsample*cfg.trllen);
                multiLockedSpikes(multiLockedSpikes_idx) = 1;
                % if number of consecutive spikes is a fraction (happens if
                % nLockedSpikes is not an integer multiple of ntroughs
                pExtraSpike = nConsSpikes - floor(nConsSpikes);
                % draw a rand from uniform distribution [0,1]
                % if rand< pExtraSpike, assign a spike
                if rand < pExtraSpike
                    multiLockedSpikes(multiLockedSpikes_idx(end)+1) = 1;
                end
                if k == length(lockedSpikes_idx)
                    break;
                else
                    k = k + 1;
                end
        ...
        lockedSpikes_idx = find(multiLockedSpikes == 1);
	 ...
	 if 0 % plot lfp and spikes (for illustration and debugging)
		 ...
		 plot(taxis(spikes_idx),-1*lfp_amp(spikePhaseFreq),'rx','MarkerEdgeColor','r'); % all spikes
...
plot(statSts.freq,statSts.ppc0', 'bo-');
xlabel('frequency');
ylabel(cfg.method);
...
disp(sprintf('PPC at 10 Hz = %0.2f', statSts.ppc0(statSts.freq==10)))
disp(sprintf('Coh at 10 Hz = %0.2f', C(end)))
disp(sprintf('spikeRate %d Hz, spikeLockProb %.2f, mean rate %.2f, mean locked rate %.2f %d spikes',spikeRate,spikeLockProb,mean(sum(Spikes,2)/1),mean(sum(lockedSpikes,2)/1),sum(sum(Spikes))));
